...
public class LoginWindow {
	...
	private void initialize() {
		...
		frmBatChat.getRootPane().setDefaultButton(btnLogin);
	...
	private class LoginListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			try{
				if(!usernameField.getText().isEmpty() && !(passwordField.getPassword().length < 4)){
					...
					JOptionPane.showMessageDialog(frmBatChat, "Your username cannot be blank, and your password must be at least 4 characters long");
			}catch(Exception ex){
				ex.printStackTrace();
			}
